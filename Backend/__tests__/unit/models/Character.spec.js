...
describe("Character", () => {
  ...
  describe("constructor", () => {
    it("creates a Character instance from database", async () => {
      const row = {
        character_id: 1,
        name: "Ghost",
        early_life: "Unknown",
      ...
      const character = await new Character(row);
  ...
  describe("getAll", () => {
    it("returns an array of Character", async () => {
      const mockRows = [
        {
          character_id: 1,
          name: "Ghost A",
  ...
  describe("getOneByID", () => {
    it("returns a Character when found", async () => {
      ...
      const result2 = await Character.getOneByID(2);
      expect(result2).toBeInstanceOf(Character);
      expect(result2.name).toBe("Ghost B");
    });
  ...
  describe("getAllByStory", () => {
    it("return all characters of a story when found", async() => {
       const mockRow = {
        story_id: 2,
        name: "Ghost B",
        early_life: "zzz",
      ...
      jest.spyOn(db, "query").mockResolvedValueOnce({ rows: [mockRow] });
        const result3 = await Character.getAllByStory(3);
      expect(result3).toBeInstanceOf(Array);
      expect(result3[0].name).toBe("Ghost B");
    });
